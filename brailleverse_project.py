...
def speak_text(text):
    ...
    tts_file = "output.mp3"
    tts.save(tts_file)
    os.system(f"start {tts_file}")  # This will play the audio file on Windows
...
def process_input_and_convert_to_braille(audio_input=False, image_upload=False, text_input=False):
    ...
    if final_text:
        ...
        save_output_to_file(braille_output)
        speak_text(braille_output)  # Optionally read the Braille output aloud
